...
- aici vom avea definite mai multe functii prin care vom modifica
contextul astfel incat
sa definim instructiunile necesare a fi indeplinite inainte de rularea
testelor si dupa
rularea testelor.

4. Fisierul browser.py:
- se mai poate denumi si driver.py
- definim o clasa numita Browser (Driver), care va avea metode si
proprietati specifice interactiunii
cu browser-ul/driver-ul (deschiderea browser-ului, setare implicit wait,
maximizare fereastra, inchidere browser etc)

5. Fisierul base_page.py:
- vom avea o clasa BasePage care va mosteni din clasa Browser si va
cuprinde proprietati si caracteristici de baza
